/**
 * Threads connection routes: status / start OAuth / callback / disconnect.
 * Structure mirrors MetrikaController: state+brandId+next in a short-lived
...
export class ThreadsController {
  ...
  async status(@CurrentUser() user: SessionUser, @Query('brandId') brandId?: string) {
    ...
    if (!brand) return { enabled: await this.threads.enabled(), connected: false, brand: null };
    const conn = await this.threads.getConnection(brand.id);
    return {
      enabled: await this.threads.enabled(),
      connected: !!conn,
  ...
  // ── Start OAuth: redirect to Threads ──────────────────────────────────────
  @Get('connect')
  @UseGuards(LoginGuard)
  async connect(
    ...
    if (!(await this.threads.enabled())) return res.redirect(`${dest}?threads_error=disabled`);

    const brand = await this.resolveBrand(user.i, brandId);
    if (!brand) return res.redirect(`${dest}?threads_error=no_brand`);
    ...
    return res.redirect(await this.threads.authUrl(state));
  }

  // ── Callback: code → short-lived → long-lived → save ──────────────────────
  @Get('callback')
  async callback(@Req() req: Request, @Res() res: Response) {
    ...
    const fail = (code: string) => res.redirect(`${dest}?threads_error=${code}`);

    const user = this.auth.userFromRequest(req);
    if (!user) return fail('auth');
    // the user may have clicked "Cancel" on the Threads screen — not an exchange error
    ...
    try {
      const short = await this.threads.exchangeCode(req.query.code as string);
      const long = await this.threads.exchangeLongLived(short.accessToken);
      // the profile isn't critical: the connection still works without it
      const profile = await this.threads
        .me(long.accessToken)
        .catch(() => ({ id: short.userId, username: '' }));
      await this.threads.upsertConnection({
        brandId: brand.id,
        userId: user.i,
      ...
      return res.redirect(`${dest}?threads_connected=1`);
    } catch {
  ...
  async disconnect(@CurrentUser() user: SessionUser, @Query('brandId') brandId?: string) {
    ...
    await this.threads.disconnect(brand.id);
    return { ok: true };
  }
}
