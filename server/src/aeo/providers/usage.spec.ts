...
describe('costUsdFor — dollar price of a usage entry', () => {
  ...
  it('success by tokens: claude-haiku 500/0 → $0.0005', () => {
    expect(costUsdFor({ provider: 'kie', model: 'claude-haiku', tokensIn: 500, tokensOut: 0, status: 'ok' })).toBe(0.0005);
  });
  ...
  it('an error with no explicit cost → null (a failed call spends no money)', () => {
    expect(costUsdFor({ provider: 'kie', model: 'claude-haiku', tokensIn: 500, tokensOut: 0, status: 'error' })).toBeNull();
  });
...
describe('recordUsage — safety (accounting must not bring down a run)', () => {
  it('does not throw without an initialized prisma (blogPrisma throw is swallowed)', () => {
    expect(() => recordUsage({ provider: 'kie', model: 'claude-haiku', tokensIn: 1, tokensOut: 1, status: 'ok' })).not.toThrow();
  });
  it('a call inside a context does not throw', () => {
    expect(() =>
      runContext('r1', () => platformContext('claude', () => recordUsage({ provider: 'kie', status: 'error', error: 'HTTP 500' }))),
    ).not.toThrow();
  });
});
