 ...
 * with a RE-parse of mentions, the aggregateFacts composition, panel syncing
 * through the store, and the note the panel-only block carries when
 * the engines stay silent.
 */
import {
  runAeoSnapshotCore,
  type AeoSnapshotDeps,
  type AeoSnapshotStore,
} from './aeo-snapshot';
...
function mkAnswers(): SnapshotAnswer[] {
  return [
    {
      platform: 'claude',
      prompt: 'best acme tool',
      text: 'acme is great, rival is ok',
...
function mkDeps(over: Partial<AeoSnapshotDeps> = {}): AeoSnapshotDeps {
  return {
    ...
    sentimentBatch: jest.fn(async () => ({
      'acme.com': { score: 70, themes: [] },
    })) as any,
    ...
    platformsFor: jest.fn(async (profile: string) =>
      profile === 'micro' ? ['claude'] : ['gemini'],
    ),
    availablePlatforms: jest.fn(async (platforms: readonly string[]) => [
      ...platforms,
    ]),
...
describe('runAeoSnapshotCore — fail-soft', () => {
  it('engines returned empty → panelOnlyBlock, run=null', async () => {
    ...
    expect((res.block as any).aeoStatusNote).toContain(
...
describe('runAeoSnapshotCore — happy path', () => {
  it('competitor validation + RE-parse of mentions + aggregateFacts', async () => {
    ...
    expect(res.competitors).toEqual([
      { domain: 'rival.com', name: null, verdict: 'direct', mentions: 2 },
    ]);
    ...
    const bf = (res.run as any).answers[0].brands_found as Array<{
      brand: string;
    }>;
    expect(bf.map((b) => b.brand).sort()).toEqual(['acme.com', 'rival.com']);

    const block = res.block as any;
    expect(block.aiBrands.map((b: any) => b.brand)).toEqual([
      'acme.com',
      'rival.com',
    ]);
    ...
    expect(deps.judgeAnswers).toHaveBeenCalledWith(
      expect.any(Array),
      'acme.com',
      ['rival.com'],
    );
    expect(deps.buildCompetitors).toHaveBeenCalledWith(
      expect.objectContaining({
        domain: 'acme.com',
        seoRows: [{ domain: 'seo1.com' }, { domain: 'seo2.com' }],
      }),
    );
  });
  ...
  it('Russian content → Yandex market (2643, ru)', async () => {
    const deps = mkDeps();
    const ru = {
      title: 'Крауд-игры и ставки на скины, много русского текста здесь',
      description: '',
      h1: '',
      hero_text: '',
    };
    await runAeoSnapshotCore(
      'shop.ru',
      mkFacts({ crawl: ru as any }),
      { ...RAW, crawl: ru } as Raw,
      {},
      deps,
    );
    expect((deps.runSnapshot as jest.Mock).mock.calls[0][3]).toMatchObject({
      locationCode: 2643,
      languageCode: 'ru',
    });
  });
...
describe('runAeoSnapshotCore — lite', () => {
  it('panel of 10, micro+mid engines, grounded=false', async () => {
    ...
    expect((deps.generatePrompts as jest.Mock).mock.calls[0][2]).toMatchObject({
      n: 10,
    });
...
describe('runAeoSnapshotCore — store (unified panel)', () => {
  it('tracker found → panel sync and aliases into the run; persistence stays post-analysis', async () => {
    ...
    expect((deps.runSnapshot as jest.Mock).mock.calls[0][3].aliases).toEqual({
      'acme.com': ['Акме'],
    });
  });
});
